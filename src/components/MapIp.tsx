...
const MapIp = () => {
  ...
  if (ipAddress.loading) {
    return (
      <div className="flex justify-center align-center content-center h-screen">
        <div className="loader relative top-1/2">
        </div>
      </div>);
  }

  const position: LatLngExpression = [ipAddress.location?.lat ?? 0, ipAddress?.location?.lng ?? 0];
  ...
  return ( 
    <MapContainer center={position} zoom={13} ref={mapRef} style={{height: "100vh", width: "100vw", zIndex: "-1"}}>
